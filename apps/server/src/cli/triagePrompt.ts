...
export const TRIAGE_PLAYBOOK = `# Agents in Cloud triage playbook

You are a support engineer for Agents in Cloud
(https://github.com/jarrodwatts/agentsin-cloud), working
...
Fetch https://raw.githubusercontent.com/jarrodwatts/agentsin-cloud/main/.github/triage/PLAYBOOK.md.
...
cache directory named in the context file, one subdirectory per commit hash:
    ...
    git clone --depth 1 --filter=blob:none --branch <release-tag> \\
      https://github.com/jarrodwatts/agentsin-cloud <source-cache-dir>/<hash>
...
Search existing issues in \`jarrodwatts/agentsin-cloud\` (use \`gh\`, or the public GitHub search API
if \`gh\` is missing or not logged in). For inherited runtime behavior, also search upstream T3 Code
issues in \`pingdotgg/t3code\`. Then check whether the problem is already fixed in a release newer
than the user's version: compare versions, read release notes and recent commits touching the
relevant code.
...
Do not patch the Agents in Cloud source as a fix. A good issue with strong repro steps
...
- If \`gh\` is not authenticated, offer \`gh auth login\`, or build a prefilled
  https://github.com/jarrodwatts/agentsin-cloud/issues/new URL with title and body query
...
export const buildTriageLaunchPrompt = (promptFilePath: string) =>
  `Read the file "${promptFilePath}" and follow its instructions exactly: it is your Agents in Cloud triage playbook, and it starts with asking the user what went wrong.`;

/** The full seed prompt, written to `prompt.md` in the triage scratch dir. */
export const buildTriageSeedPrompt = (contextFilePath: string) => `An Agents in Cloud user is \
...
export const buildTriageContext = (input: TriageContextInput) => `# Agents in Cloud triage context
...
- Repo: https://github.com/jarrodwatts/agentsin-cloud
