...
export const makeProviderCredentialRpc = (input: {
  ...
  const adapter = makeHostedAgentConnectionAdapter({
    logins: input.logins,
    profiles: input.service,
  });
  return {
    handleHttp: (request: Request): Effect.Effect<Response | undefined> => {
      ...
      return Effect.gen(function* () {
        ...
        const workspace = yield* input.workspaces
          ...
          .pipe(
            Effect.mapError(() => new ProviderCredentialRpcError({ code: "workspace_failed" })),
          );
        if (workspace === undefined) return response({ error: "not_found" }, 404);
        const workspaceId = yield* decoders
          .workspaceId(workspace.id)
          .pipe(
            Effect.mapError(() => new ProviderCredentialRpcError({ code: "workspace_invalid" })),
          );
        ...
        if (pathname.endsWith("/login/poll")) {
          ...
          return response(yield* adapter.pollLogin(principal, command.loginId));
        }
        if (pathname.endsWith("/login/cancel")) {
          const command = yield* body(request, decoders.cancel);
          return response(yield* adapter.cancelLogin(principal, command.loginId));
        }
        if (pathname.endsWith("/profiles/seal")) {
          const command = yield* body(request, decoders.seal);
          return response(
            yield* adapter.sealProfile(principal, {
        ...
        if (pathname.endsWith("/profiles/materialize")) {
          ...
          return response(
            yield* adapter.materialize(principal, {
              threadId: command.threadId,
              profileId: command.profileId,
        ...
        if (pathname.endsWith("/profiles/validate")) {
          ...
          return response(yield* adapter.validate(principal, command.profileId));
        }
        if (pathname.endsWith("/profiles/refresh")) {
          const command = yield* body(request, decoders.profile);
          return response(yield* adapter.refresh(principal, command.profileId));
        }
        if (pathname.endsWith("/profiles/revoke")) {
          const command = yield* body(request, decoders.profile);
          return response(yield* adapter.revoke(principal, command.profileId));
        }
